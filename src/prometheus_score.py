...
def prometheus_ask_score(original,reference,correction, tokenizer, model):
    text = {
            "ocr": original,
            "gold": reference,
    ...
    score = valuta_judge(text,model,tokenizer)
...
def prometheus_score(FILE_NAME, correction_model):
    ...
    for i, entry in enumerate(data, start=1):
        ...
        try:
            score = prometheus_ask_score(original,reference,correction, tokenizer, model)
